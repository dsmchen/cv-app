# CV App

[The Odin Project: CV Application](https://www.theodinproject.com/lessons/node-path-react-new-cv-application)

## Resources

- Font: [Google Fonts - Inclusive Sans](https://fonts.google.com/specimen/Inclusive+Sans)
- Icons: [Pictogrammers - Material Design Icons](https://pictogrammers.com/library/mdi/)
- CSS Reset: [Joshua Comeau - A Modern CSS Reset](https://www.joshwcomeau.com/css/custom-css-reset/)
- Colours: [Material Design - The Color System](https://m2.material.io/design/color/the-color-system.html)
- Accessibility:
  - [DEV - Making Error Messages Accessible](https://dev.to/hellonehha/making-error-messages-accessible-28i1)
  - [W3C - Accordion Example](https://www.w3.org/WAI/ARIA/apg/patterns/accordion/examples/accordion/)

## Out of scope

- Responsive design
